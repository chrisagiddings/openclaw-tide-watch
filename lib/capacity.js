...
// Gateway status cache (prevents blocking dashboard refresh in watch mode)
let gatewayStatusCache = null;
let lastGatewayCheck = 0;
const GATEWAY_CHECK_INTERVAL = 60000; // Check every 60 seconds (not every 10s)

/**
 * Check OpenClaw gateway status (with caching to prevent blocking)
 * @param {boolean} skipCache - Force a fresh check (default: false)
 * @returns {Object} Gateway status { online: boolean, status: string, emoji: string }
 */
function checkGatewayStatus(skipCache = false) {
  const now = Date.now();
  
  // Return cached status if checked recently (prevents blocking on every refresh)
  if (!skipCache && gatewayStatusCache && (now - lastGatewayCheck < GATEWAY_CHECK_INTERVAL)) {
    return gatewayStatusCache;
  }
  
  // Perform actual check (may block for up to 500ms)
...
/**
 * Perform actual gateway status check (blocking)
...
function performGatewayStatusCheck() {
  ...
  try {
    // Try to run openclaw gateway status (reduced timeout for faster failure)
    const output = execSync('openclaw gateway status', { 
      encoding: 'utf8',
      timeout: 500,  // Reduced from 5000ms - fail fast
    ...
    // If command fails or times out, return cached status or offline
    // This prevents blocking if gateway is slow/unresponsive
    if (gatewayStatusCache) {
      // Return last known status with staleness indicator
      return {
        ...gatewayStatusCache,
        stale: true
      };
