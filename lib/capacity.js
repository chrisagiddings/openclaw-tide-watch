...
function formatDashboard(sessions) {
  ...
  lines.push('Session ID  Channel/Label     Model                      Capacity                    Tokens        Last Active');
  lines.push('─'.repeat(120));
  ...
  sorted.forEach(session => {
    ...
    lines.push(`${id}  ${channelLabel}  ${model}  ${emoji} ${bar} ${pct}  ${tokens}  ${lastActive}`);
  });
...
function checkGatewayStatus() {
  ...
  try {
    // Try to run openclaw gateway status
    ...
    // If command fails, gateway is likely offline or not responding
    return {
      online: false,
      status: 'Offline',
      emoji: '🔴',
