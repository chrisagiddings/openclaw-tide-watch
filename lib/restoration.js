 ...
 */
function isDiscoveryEnabled(restoreDir = DEFAULT_RESTORE_DIR) {
  const markerPath = path.join(restoreDir, DISCOVERY_MARKER);
 ...
 */
function disableDiscovery(restoreDir = DEFAULT_RESTORE_DIR) {
  // Ensure directory exists
  if (!fs.existsSync(restoreDir)) {
    fs.mkdirSync(restoreDir, { recursive: true });
  }
 ...
 */
function enableDiscovery(restoreDir = DEFAULT_RESTORE_DIR) {
  const markerPath = path.join(restoreDir, DISCOVERY_MARKER);
 ...
 */
function getDiscoveryStatus(restoreDir = DEFAULT_RESTORE_DIR) {
  const enabled = isDiscoveryEnabled(restoreDir);
  const markerPath = path.join(restoreDir, DISCOVERY_MARKER);
